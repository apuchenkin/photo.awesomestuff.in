...
const env = process.argv.includes('--release')
  ? 'production'
  : 'development'
;
...
gulp.task('serve', ['build-client', 'build-server'], () => {
  nodemon({
...
gulp.task('build-server', ['build-client'], () => gulp
  .src(`${SRC}/server.js`)
  .pipe(runWebpack(serverConfig(env), webpack))
  .pipe(gulp.dest(`${DIST}/`)),
);

gulp.task('build-client', () => gulp
  .src(`${SRC}/index.js`)
  .pipe(runWebpack(clientConfig(env), webpack))
